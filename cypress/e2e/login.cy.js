...
describe('Login spec', () => {
  ...
  it('should display Alert when username is empty', () => {
    ...
    // memverifikasi window.alert untuk menampilkan pesan dari AP
    cy.on('window:alert', (str) => {
      expect(str).to.equal('"id" is not allowed to be empty');
    });
  });

  it('should display Alert when pasword is empty', () => {
    // mengisi username
  ...
  it('should display alert when username and password are wrong', () => {
    ...
    cy.get('input[placeholder="Username"]').type('testuser');
    // mengisi password
    cy.get('input[placeholder="Password"]').type('blabla123');
