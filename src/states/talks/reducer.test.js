...
describe('talksReducer Function', () => {
  ...
  it('should return the talks when  givent by RECEIVE_TALKS action', () => {
    // arrange
    const initialState = [];
    const action = {
      type: 'RECEIVE_TALKS',
  ...
  it('should return the talks with the toggle like talk when given by TOGGLE__LIKE_TALK action', () => {
    // arrange
    const initalState = [
      {
        id: 'talk-1',
    ...
    // assert
    expect(nextState2).toEqual(initalState);
  });
});
